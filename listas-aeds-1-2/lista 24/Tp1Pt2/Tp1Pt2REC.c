...
int calcularAaprovados(int qtd, float notas[qtd]){
    
    if (qtd <= 0){
        return 0;
    }

    return (notas[qtd - 1] >= 60 ? 1 : 0) + calcularAaprovados(qtd - 1, notas);
}
        ...
        switch(opcao){
            ...
            case 2:{
                if (notas == NULL) {
                    ...
                    int aprovados = calcularAaprovados(quantidade, notas);
